...
| 32 (08:00Z, user report) | A one-run track's result names DRAGSTER because the title comes from the DRAGSTER assets | EAST held-Right capture to its result (native exact on all 4,178 rows to the finish); the title from the pack's name table, centred (first 16 - length, corrected in review to 16 - width/2 in tiles); stable result frames 6,200-6,598 against the original, DRAGSTER's result as the baseline | Title glyphs 0 differing pixels; the rest is the result screen's existing residue (DRAGSTER's own: 26,244-26,376 px). The review's withheld FLAT FUN returned the underscore (one tile, not two) and the centring (by width in tiles); after the fix its title rows differ by 0 px, and f, u and n are confirmed | Pull request #14 |
