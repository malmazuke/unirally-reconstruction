...
| 32 (08:00Z, user report) | A one-run track's result names DRAGSTER because the title comes from the DRAGSTER assets | EAST held-Right capture to its result (native exact on all 4,178 rows to the finish); the title from the pack's name table and centred (16 - length); stable result frames 6,200-6,598 against the original, DRAGSTER's result as the baseline | Title glyphs 0 differing pixels; the rest is the result screen's existing residue (DRAGSTER's own: 26,244-26,376 px) | Tier 2 pull request; letters outside the observed twelve and the underscore are the font layout's reading |
