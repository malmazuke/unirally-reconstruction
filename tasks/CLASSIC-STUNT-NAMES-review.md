...
# CLASSIC-STUNT-NAMES - fourth round: the gate shortcut, and whether this can be integrated

- Candidate: `bf16faf9a360b81fa283cad7db711889cee22230`, one commit on top of `6cbef9d` (the third
  round's candidate), two on top of `6e0fad6` and three on top of `f2a4b1f`. Base `main` at
  `fd34209`. `bf16faf` rewrites `tools/unirally_lab/native/gate_identity.py` and applies the third
  round's E3 to E10: six files, 135 insertions, 64 deletions, no binaries.
- Reviewing model: Claude Opus 5, fresh session, no inherited conversation with the primary and
  none with any of the three earlier reviewers. The third round's E1 to E10, above, are this
  round's specification.
- Checkout: `.worktrees/classic-stunt-names-review4`, branch `review/classic-stunt-names-4`,
  created at the exact candidate. Every number below was measured there against a build made
  there. Nothing on `task/classic-stunt-names`, in the primary's worktree, in any of the three
  earlier review worktrees or in the main checkout was modified; all five were read only.
- ROM `a1105819d48c04d6...` and pack `d7c96d89882d2489...`, both hashed in this checkout at the
  start of the gate run. The pack digest is the one every gate report of this task carries.
- Scope: the third round said acceptance may not rest on `gate_identity.py`, and the primary used
  that tool to cite the eleven differential gates at `bf16faf` rather than run them. So this round
  did two things. It attacked the rewritten tool with eighteen probes, every source mutation
  **committed** rather than left in the working tree, because the new dirty-tree refusal would
  otherwise stand in for the check actually under test. And then it **ran all eleven differential
  gates at `bf16faf`** anyway, so that the verdict does not depend on the shortcut being right. I
  re-derived none of the recovery: the table, the encoding, the pack extraction, the alphabet from
  the ROM, the queue probes, the blanking rule and the window-member order were reproduced by the
  first two rounds and this candidate changes none of that code or content.
- Verdict: **approve**, with seven should-fix and seven advisory items and no blocking finding.
  E1 is fixed and I could not falsify the rewritten tool in any way that moves the gate binary's
  behaviour. E2 to E10 are addressed, with residuals named below. The E9 bitset change is
  pixel-neutral over the whole frame, not just the caption band. And acceptance does not in fact
  rest on the shortcut, because the eleven gates ran here and returned the same restore counts and
  the same `rows_sha256` digests as the reports at `6e0fad6`.

## What I ran

```sh
PATH="$PWD/local/toolchain/ninja-1.13.2-darwin-arm64:$PATH" python3 tools/project.py build --preset app-debug
python3 -m tools.unirally_lab.native.gate_identity --since 6e0fad6 --reports <copy of the primary's gates-6e0fad6> \
    --pack local/classic-pal-crawler-two-tracks-v9.pack --ninja local/toolchain/ninja-1.13.2-darwin-arm64/ninja
bash    artifacts/classic-stunt-names-review4/gate_identity_probe4.sh   # mine: eighteen attacks, committed
bash    artifacts/classic-stunt-names-review4/mutation_check4.sh        # mine: six mutations of the caption rule
python3 artifacts/classic-stunt-names-review4/band_sweep.py             # the re-review's sweep, at this candidate
bash    artifacts/classic-stunt-names-review4/bitset_neutral.sh         # mine: E9 full-frame, bf16faf against 6cbef9d
python3 artifacts/classic-stunt-names-review4/caption_domain.py         # mine: the whole reachable caption domain
python3 artifacts/classic-stunt-names/{caption_pictures,zoom_captions,trick_pictures,caption_alphabet}.py
bash    artifacts/classic-stunt-names-review4/gates4.sh                 # the full regression, all eleven gates
```

My artifacts are in this checkout's ignored `artifacts/classic-stunt-names-review4/`: the
re-review's `band_sweep.py` copied in unchanged; my own `gate_identity_probe4.sh`,
`mutation_check4.sh`, `bitset_neutral.{py,sh}`, `caption_domain.py` and `gates4.sh`; and the logs
and reports under `gates/`. The only things I read from other checkouts are the three earlier
rounds' reports, the primary's `gates-6e0fad6` reports and its four caption scripts, and the frame
images of its `pictures-a`, `trick-roll` and `trick-twist` captures, all read only.

## 1. E1, the blocking one: is the rewritten `gate_identity.py` sound?

**What changed.** The object list is no longer three hardcoded strings. `engine_files` asks
`ninja -t inputs src/core/zoom_zoo_runner`, keeps the `.o` lines, and unions each object's recorded
dependencies. In this checkout that is **9 objects and 19 repository files** - the nine translation
units and ten headers - where the old version compared 13 files and none of the six engine
implementations. It now also refuses a failed `ninja` query, a dependency record that is not
`VALID`, a file absent at the cited commit, an uncommitted tracked change, and a report whose
`status`, `source_commit`, `pack_sha256` or `contract_sha256` does not match.

**The attack.** Round three's reproduction left its mutation in the working tree. That no longer
tests anything, because the dirty-tree refusal fires first and the file comparison is never
reached. So every source probe below is **committed on a throwaway branch**, built where a build
is the point, and reverted; `gate_identity_probe4.sh` restores the tree and rebuilds on exit, and
the log's last line records `tracked changes 0` and the baseline rows digest restored.

```
baseline zoom_zoo_runner rows sha 8e89b575a49078be      (round three measured the same value)
```

| # | attack | result |
| --- | --- | --- |
| P1 | round three's killer, `track_sampling.cpp:12` `sampling_x_bias 8 -> 9`, uncommitted | refused: uncommitted tracked changes |
| P2 | **the same edit, committed** | **refused**: `changed since 6e0fad6: src/core/track_sampling.cpp`; rebuilt rows move `8e89b575a49078be -> 258979f87f96a968` |
| P3 | header-only, comment-only edit to `movement.hpp`, committed | refused: `changed since 6e0fad6: src/core/movement.hpp` - the comparison is byte-level, so it is conservative |
| P4 | header edit that moves rows, `dragster_race_state_magic` `0001 -> 0002`, committed | refused; rebuilt rows move to `4962b915f58f69f3` |
| P5 | `src/core/engine_tweak.cpp` added to `unirally_movement`, committed **and rebuilt** | refused: `10 objects, 20 repository files`, `absent at 6e0fad6, so new to this tree: src/core/engine_tweak.cpp` |
| P6 | the same addition, committed and **not** rebuilt | **cited, rc=0** - see S1 |
| P7 | `CMakePresets.json` switches `app-debug` to RelWithDebInfo, committed and rebuilt | cited, rc=0; declared out of scope - and the rebuilt rows are `8e89b575a49078be`, unmoved |
| P8 | `build/app-debug/.ninja_deps` removed | refused: `flat_contact.cpp.o has no valid recorded dependencies` |
| P9 | one object touched so its record goes STALE | refused, naming that object |
| P10 | `--pack` pointed at the v8 pack | refused, all eleven: `ran against a different pack` |
| P11 | a frozen contract edited and committed | refused: `m4-16-primary: its frozen contract is not in the tree unchanged` |
| P12 | four doctored reports: `status`, `source_commit`, `pack_sha256`, `contract_sha256` | all four named and refused |
| P13 | `--reports` holding one of the eleven | **cited, rc=0**, `1 gates may be cited` - see S2 |
| P14 | an empty report directory | refused: `no differential gate reports in ...` |
| P15 | every report's `source_diff_sha256` set to a non-empty digest | **cited, rc=0** - see S3 |
| P16 | a listed engine source deleted and committed | `FileNotFoundError` traceback, rc=1 - fails closed, ungracefully |
| P17 | `--build build/does-not-exist` | refused: `ninja -t inputs ... failed` |
| P18 | the docstring's premise, checked with `nm -C` | **0** of the gate binary's 1,621 symbols match presentation, `render_classic` or caption |

**The judgement.** The round-three defect is gone, and I could not construct a case where the tool
licenses citing while the gate binary's behaviour moves. Every change to any of the nineteen files
is caught, source or header, behavioural or cosmetic. A file new to the build graph is caught once
the tree is built. The pack, the contracts and each report's own status and commit are checked.
And the soundness no longer depends on the "no presentation symbol" claim at all: the input set
now comes from the build graph, so if presentation were ever linked into the binary its sources
would simply appear in the comparison. **Acceptance may rest on it** - with the three residuals
below fixed, and with the standing caveat that a shortcut should never be the only thing between a
candidate and its evidence. It was not here: I ran the eleven gates.

**S1, the one fail-open I did find.** P6: commit a new translation unit into a library the binary
links and do not rebuild, and the tool queries the previous build's `build.ninja`, still sees nine
objects and nineteen files, and prints "every input is byte identical and every report matches; 11
gates may be cited". Nothing checks that the build directory is current with the tree. I judge this
**should-fix rather than blocking**, for a reason worth stating: the escape only exists for a file
the build graph has never seen, and a new translation unit cannot change the engine's behaviour
without something calling it - which means editing one of the nineteen files, which is caught
whatever the build state, because the comparison is against the tree rather than against anything
the build produced. The remedy is one line: `ninja -C <build> -n src/core/zoom_zoo_runner` prints
"no work to do" exactly when the build is current, and anything else is a refusal.

**S2.** P13: the tool validates each report it is given and never asks whether the set is complete.
Pointed at a directory holding one of the eleven it prints "1 gates may be cited" and returns 0.
The acceptance row names eleven; the tool cannot tell eleven from one.

**S3.** P15: `source_diff_sha256` is still unchecked, as round three's E2 said. A gate report whose
own run had a dirty tree is cited without complaint, which defeats the premise that `source_commit`
pins the sources. All eleven real reports carry `e3b0c442...`, the empty-tree digest, so this
instance is clean - I checked - but the field is there to be read.

**S4.** `docs/BUILD_AND_VALIDATION.md:560-584` was not updated when the tool was. The published
command is now wrong in a way that simply fails:

```
$ python3 -m tools.unirally_lab.native.gate_identity --since 6e0fad6 \
      --reports artifacts/<task>/gates-<that commit> --ninja local/toolchain/.../ninja
error: the following arguments are required: --pack
```

The section also still says "13 repository files at present" where the tool reports 19, still
describes only the byte comparison and none of the report checks or the clean-tree requirement,
and still names only the toolchain and system headers as out of scope where the module's own
docstring also excludes the CMake files and presets. This is the procedure every later task will
copy.

**S5.** The tool has no tracked test. It is a check whose entire job is to fail closed, in a
repository that mutation-checks its renderer tests, and nothing would notice if a later edit made
`engine_files` return an empty union again. A test that commits nothing - point `--since` at a
commit where a listed file differs and require a refusal - would pin the behaviour this round
measured.

**A1.** The docstring's "This checks that rather than asserting it" reads as a claim that the tool
verifies the no-presentation-symbol property. It does not run `nm`. The property is true today
(P18) and, as above, is no longer load-bearing, so the sentence should be reworded rather than the
check added.

**A2.** P16's deleted-file path raises `FileNotFoundError` instead of refusing; it is reachable
only for a file deleted in a commit, since the dirty check covers the uncommitted case, and it
does exit non-zero. `engine_files` is annotated `-> list[str]` and returns a tuple.

## 2. E2 to E10, item by item

| item | state at `bf16faf` |
| --- | --- |
| E2 what the tool does not check | **mostly applied.** `status`, `source_commit`, `pack_sha256` and `contract_sha256` are all checked, and I made each one fail on demand (P10 to P12). The working-tree/`HEAD` confusion is resolved by the new dirty refusal, and the `VALID`/`STALE` marker is honoured (P9). Three residuals stand: `source_diff_sha256` (S3), report-set completeness (S2), and the documentation (S4). The CMake files and presets are now named out of scope in the module, though not in the document |
| E3 the punctuation tiles in R-0042 | **fixed.** `R-0042:43-47` records `!` = `$60`, `"` = `$61` "which is how the table spells an apostrophe", `-` = `$4D`, and that entries 1-255 hold 29 distinct bytes with no `q` and no digit although `q` has a tile at `$2B`. I read all three tiles out of the pack's own font sheet: section 4 |
| E4 attempts 15 to 18 cited but missing | **applied, with a formatting defect.** Rows 15 to 19 now exist. A blank line at `tasks/CLASSIC-STUNT-NAMES.md:115` splits the attempts table in two, and rows 15-19 have no header or delimiter row of their own, so that half does not render as a table at all - it renders as a paragraph of pipe characters. `## Handoff` then follows row 19 with no blank line |
| E5 the pack stem that cannot exist | **half applied.** `src/app/sdl_main.cpp:257` now says `local/classic-pal-crawler-two-tracks-v9.pack`, which is the file that exists. `src/app/README.md:26` was not touched and still tells the reader the launcher uses "`local/classic-crawler-two-tracks.pack` (or `-v9.pack`)". The v8 file really is `local/classic-crawler-two-tracks-v8.pack`, so the stem changed with the profile and the `-v9` form of that sentence names nothing |
| E6 the addend is CGRAM 27 | **applied in the record, not in the code.** `R-0042:59-65` now says 13 is `$000d`, CGRAM 27 on both race palettes, the colour the recovered `$3800` attribute selects, and keeps "which PPU configuration selects between them is still not recovered". `src/core/presentation.cpp:1722-1723` still carries the superseded sentence - "why the added 13 is not half of the ink's own 5-bit 28, are not recovered" - so the renderer's own comment tells a reader the thing the record has since explained. E6's second limb, that "Which BG layer carries the caption is not established" understates the first round's `$2109 = $18` / `$2105 = $19` measurement, was not applied |
| E7 the withheld frames credited for the blend | **fixed.** The composition row now cites the 274-frame sweep alone, and a new row records the third round's census and states in bold that every blend in evidence is on ZOOM ZOO, with the eight DRAGSTER trick frames carrying 999 flat caption pixels and no blend |
| E8 attempt order and the duplicated phrase | **fixed.** Attempt 12 precedes 13, and "the original's pale pink - the original's pale pink" is now written once |
| E9 the 57 KB stack array | **fixed, and pixel-neutral - section 3** |
| E10 the table-size guard | **fixed.** Section 5 |

## 3. E9: is the bitset change pixel-neutral?

`std::array<bool,256*224> caption_ink{}` became `std::bitset<256*224> caption_ink`, with
`inked[i]=true` becoming `inked.set(i)` and `!caption_ink[at]` becoming `!caption_ink.test(at)`.
The primary says the picture scores and the 274-frame sweep are unchanged. The sweep only looks at
the caption band, but `caption_ink` is consulted for **every rider pixel anywhere on screen**, so a
band-only check would not see an indexing difference outside it. I compared the whole frame.

`bitset_neutral.sh` builds this checkout at `bf16faf`, renders every kept frame of the M4-16 ZOOM
ZOO original to a PPM, checks out `6cbef9d` - the `std::array<bool>` version - builds and renders
the same frames, restores, and diffs the complete images:

```
bf16faf: rendered 274 frames
6cbef9d: rendered 274 frames        (grep confirms std::array<bool,256*224> caption_ink{} at 1724)
274 frames compared full-frame (47140878 bytes each side); 0 differ; worst differing pixels 0
```

Forty-seven megabytes of rendered pixels, byte for byte identical. And the sweep itself, run here
against my own build at the candidate:

```
274 kept frames compared; worst caption-band mismatch 0
27 of them have a channel-6 member covering caption-band pixels   (members 0 to 21, all 2304/2304)
```

## 4. My withheld case: the domain, not the frames

Three rounds have checked the frames the captures happen to show. The renderer can be driven into a
much larger domain than that: the engine publishes event ids 1 to 255, `classic_caption_tile`
**throws** on a byte it does not know, and the first round's blocking finding was exactly a race
ended by that throw. So I walked the whole reachable domain instead, replicating the recovered
arithmetic in Python and asking the pack's own font sheet whether each resulting tile pair actually
holds a glyph (`caption_domain.py`).

```
pack entries validated: 57; caption table 4080 bytes, font sheet 2048 bytes
table is byte-identical to the ROM at $0BCA04: True        ($17:C9F4 + 16; entry 0 is not text)
events walked: 1-255, 255 entries, 4080 glyph slots
bytes the renderer would throw on: 0
tile pairs outside the 128-tile sheet: 0
distinct top tiles used: 28; empty halves among them: [77, 113]
```

Three things fall out of that.

**The throw is unreachable for this pack.** All 4,080 bytes of all 255 entries map, and every
resulting top tile and its `+$10` partner lie inside the 128-tile sheet. The pack entry is
byte-identical to the ROM at `$17:C9F4 + 16`, which is the stride the renderer's
`subspan((event-1)*16,16)` assumes, and the ROM's entry 0 at the base itself is not text at all -
`æ\nç\nè\né\nê\në\nÿÿ\0\0` - which corroborates that the usable domain starts at 1.

**The voice range 72-87 renders.** This was the first round's blocking finding, and the limit
R-0042 carries is that no capture *displays* one. All sixteen entries map without a throw, the
three that hold `"` among them:

```
  event  72 |    rockin      |     event  75 |   thrashin"    |     event  79 |    jammin"     |
  event  80 | head bangin"   |     event  85 |                |     event  86 | huh  huh  huh  |
```

**The punctuation tiles E3 added to R-0042 are readable in the sheet.** No original picture in
evidence shows one, so the claim rested on the primary having looked. Drawing them out of the
pack's own font:

```
  '"'  top $61 / bottom $71      '-'  top $4D / bottom $5D      '!'  top $60 / bottom $70
       ..##....   (bottom half         ........                      ..##....
       ..##....    entirely            ..#####.  <- row 9 of 16      ..##....
       ...#....    blank)              ..#####.                      ..##....
       ..#.....                        ........                      ..#.....
                                                                     .###....  <- the dot
```

An apostrophe that occupies only the top half, a hyphen at exactly mid-height, an exclamation mark
with its stem and dot. Those are the two "empty halves" my check flagged - tile 77 is `-`'s blank
top and tile 113 is `"`'s blank bottom - and they are what punctuation ought to look like, not a
mapping error. `j`, `k`, `q`, `x` and `z`, the five letters no picture has exercised, are all
unmistakable letterforms at `$24`, `$25`, `$2B`, `$42` and `$44`.

## 5. E10: the size guard, mutation-checked

Round three killed three mutants and one survived. Six mutants here, each built and run through
`ctest` on app-debug and then reverted (`git status` empty afterwards, rebuilt):

```
killed: ignore empty_display (the blanking rule's second half)     -> 96% tests passed, 1 of 23 failed
killed: draw on event 0 (no event published yet)                   -> 96% tests passed, 1 of 23 failed
killed: off-by-one in the table index                              -> 96% tests passed, 1 of 23 failed
killed: accept a table of any size (round three's survivor)        -> 96% tests passed, 1 of 23 failed
killed: accept any table of at least 16 bytes                      -> 96% tests passed, 1 of 23 failed
MUTATION SURVIVED: accept a table that is too LONG
```

E10 is addressed: the mutant round three found alive is dead, and so is a stronger variant of it.
The new test drives `classic_caption_entry` with a 4,064-byte table, so it pins the short side of
`captions.size()!=4080` and not the long side; a pack entry with extra bytes would still be
indexed. That is a one-line addition, not a defect in the rule.

## 6. Regression: all eleven differential gates, run rather than cited

The candidate is presentation, records, tests and tooling, and the primary cited the eleven gates
through `gate_identity`. I ran them. Everything below is from this checkout at `bf16faf` with a
clean tracked tree (`dirty 0` in the log header), on the v9 pack
`d7c96d89882d2489...` and ROM `a1105819d48c04d6...`, the compares one at a time,
`00:15:20Z` to `00:52:18Z`.

- **Five preset builds and `ctest`: 23/23 on all five** - lab-debug, lab-release, lab-sanitize,
  app-sanitize, app-debug. Both sanitizer presets matter for the bitset change.
- **Both v1 presentation contracts** on the legacy DRAGSTER renderer with the `dragster.v1` pack:
  `winner status=passed`, `loser status=passed`.
- **Hidden app runs**, 4000 updates each on the v9 pack: DRAGSTER and ZOOM ZOO both `rc=0`,
  rider-pose fallback frames **0**.
- **Fuzz** (lab-release, v9 pack, 40 seeds): `rc=0`, **382535 updates, 79 completed races, 1242
  pause restarts, 7696 renders, 0 aborts** - byte-identical to the primary's and to all three
  earlier rounds.
- **Caption picture scores**, all four of the scripts the acceptance row names:
  `caption_pictures` 1601/1637/1652/1685 each 2304/2304, worst 0; `zoom_captions` 1450, 1583,
  1649, 3208, 4840, 6484 each 2304/2304; `trick_pictures` 2599, 2605, 2615, 2630, 2905, 2911,
  2920, 2940 each 2304/2304, worst 0; `caption_alphabet` 255 entries, 29 distinct bytes, none
  outside the alphabet. The 274-frame sweep is in section 3: worst mismatch 0.

| gate | mine at `bf16faf` | the primary's at `6e0fad6` |
| --- | --- | --- |
| m4-16-primary | passed, 757 restores, rows `b4a34af722b26693` | passed, 757, `b4a34af722b26693` |
| m4-16-idle | passed, 801, `205d1705b6791100` | passed, 801, `205d1705b6791100` |
| opposing-ride | passed, 801, `205d1705b6791100` | passed, 801, `205d1705b6791100` |
| opposing-axes | passed, 781, `3d101ea649550148` | passed, 781, `3d101ea649550148` |
| opposing-edges | passed, 757, `b4a34af722b26693` | passed, 757, `b4a34af722b26693` |
| dragster-primary | passed, 379, `dac612cc71a1a215` | passed, 379, `dac612cc71a1a215` |
| dragster-random-1 | passed, 567, `7df2f728949c6ed9` | passed, 567, `7df2f728949c6ed9` |
| dragster-random-3 | passed, 493, `391f008e00c7d889` | passed, 493, `391f008e00c7d889` |
| dragster-reversal | passed, 327, `f142311ef5a0fff4` | passed, 327, `f142311ef5a0fff4` |
| dragster-regression-landing-held-roll | passed, 181, `8c3022d0716847b7` | passed, 181, `8c3022d0716847b7` |
| dragster-regression-countdown-actions-tie | passed, 179, `3a68c24684657bc9` | passed, 179, `3a68c24684657bc9` |

**11 of 11 identical in status, restore count and `rows_sha256`.** So the shortcut's answer was
right here, and it is now a measurement rather than an inference. Note also that my gate binary
hashes `3f8733109295...` against the reports' `4751c0bf87d1...` while every row digest matches
exactly: that is the tool's own reason for comparing inputs rather than binaries, demonstrated.

No accepted contract moved, and I found no changed baseline, weakened comparison, masked skip or
accidental content commit; the candidate's diff is six files, 135 insertions, 64 deletions, no
binaries.
...
**None.** Round three's E1 is fixed, the rewritten tool survived eighteen attacks, and the eleven
differential gates it was used to cite were run here and agree exactly.

### Should-fix

**F1. `gate_identity` fails open on a stale build directory.** Commit a new translation unit into
a library `zoom_zoo_runner` links, do not rebuild, and the tool queries the previous build's graph,
still reports "9 objects, 19 repository files" and prints "every input is byte identical and every
report matches; 11 gates may be cited", rc=0. Reproduction: P6 in
`artifacts/classic-stunt-names-review4/gate_identity_probe4.sh`. Nothing checks that the build
directory is current with the tree. Not blocking because the escape exists only for a file the
graph has never seen, and such a file cannot change behaviour without an edit to one of the
nineteen - which is caught whatever the build state. Remedy: refuse unless
`ninja -C <build> -n src/core/zoom_zoo_runner` says there is no work to do.

**F2. It never asks whether the cited set is complete.** P13: pointed at a directory holding one of
the eleven reports it prints "1 gates may be cited" and returns 0. The acceptance row names eleven
gates; the tool cannot tell eleven from one. Either take the expected names, or print the count and
require it.

**F3. `source_diff_sha256` is still unchecked** (round three's E2, unapplied). P15: set every
report's to a non-empty digest and all eleven are still cited. A gate that ran against a dirty tree
is not pinned by its `source_commit`, which is the whole premise. All eleven real reports carry the
empty-tree digest `e3b0c442...`, so this instance is clean.

**F4. `docs/BUILD_AND_VALIDATION.md:560-584` was not updated with the tool.** The published command
omits the now-required `--pack` and exits 2 with an argparse error; the text still says "13
repository files at present" where the tool reports 19; it describes the byte comparison and none of
the report, clean-tree or dependency-record checks; and it names only the toolchain and system
headers as out of scope where the module also excludes the CMake files and presets. This section is
the procedure every later task will copy.

**F5. `gate_identity` has no tracked test.** A check whose whole job is to fail closed, in a
repository that mutation-checks its renderer tests, with nothing to notice if a later edit made the
derived input set empty again.

**F6. E5 is half applied: `src/app/README.md:26` still names a pack file that cannot exist.**
`sdl_main.cpp:257` was fixed; the README still tells the reader the launcher uses
"`local/classic-crawler-two-tracks.pack` (or `-v9.pack`)". The v8 file really is
`local/classic-crawler-two-tracks-v8.pack`, so the stem moved with the profile, and the `-v9` form
of that sentence names nothing that exists or can be produced - `tools/unirally_lab/frontend/commands.py`
derives the path from the profile id, so the real file is `local/classic-pal-crawler-two-tracks-v9.pack`.

**F7. The record still presents `6e0fad6` as the candidate, and never mentions `bf16faf`.**
`tasks/CLASSIC-STUNT-NAMES.md:5` reads "matrix green at `6e0fad6` ...; a third review round is
running"; the handoff says "two review rounds so far ... a third round is due", "Verified findings:
attempts 1 to 18" where the table now has nineteen rows, and lists only `gates-{a,b}.log` under
commands executed. Nowhere does it say that at the candidate the eleven differential gates were
**cited from `6e0fad6` rather than run**, which is exactly what a fresh agent would need to know to
judge the evidence. AGENTS.md requires the handoff to carry the actual commit, commands and
results. (My run above supplies the missing measurement; the record still has to say so.)

Also in this class, and cheap: the blank line at `tasks/CLASSIC-STUNT-NAMES.md:115` splits the
attempts table in two, so rows 15 to 19 render as a paragraph of pipe characters rather than table
rows, and `## Handoff` follows row 19 with no blank line.

### Advisory

**A1. The renderer's comment still tells the superseded story.** `src/core/presentation.cpp:1722-1723`:
"which PPU configuration produces it, and why the added 13 is not half of the ink's own 5-bit 28,
are not recovered". R-0042 now explains the 13 (E6: CGRAM 27, the `$3800` attribute's own colour);
the code comment beside the arithmetic does not.

**A2. E6's second limb was not applied.** R-0042:124 still says "Which BG layer carries the caption
is not established" although the first round measured `$2109 = $18` and `$2105 = $19` on frame 1249.
Leaving it is defensible; saying so is not the same as saying nothing was measured.

**A3. The docstring's "This checks that rather than asserting it"** reads as a claim that the tool
verifies the no-presentation-symbol property. It does not run `nm`. The property holds - 0 of the
binary's 1,621 symbols match presentation, `render_classic` or caption - and it is no longer
load-bearing, since the input set now comes from the build graph. Reword rather than add a check.

**A4. A deleted listed file raises instead of refusing.** P16: `FileNotFoundError` traceback rather
than a refusal message, rc=1 so it still fails closed. Reachable only for a file deleted in a
commit. `engine_files` is annotated `-> list[str]` and returns a tuple.

**A5. A CMake or preset change is out of scope and undetected.** P7: switching `app-debug` to
RelWithDebInfo, committed and rebuilt, is still cited. The module declares this, and the rebuilt
rows were `8e89b575a49078be`, unmoved - which is also mild evidence that the engine has no
optimisation-sensitive undefined behaviour. Worth naming in the document (F4) rather than fixing.

**A6. The size-guard test pins only the short side.** The surviving mutant accepts an over-long
table. One more `require` with a 4,096-byte vector closes it.

**A7. `classic_caption_entry` indexes a 32-element array with an 8-bit cursor and no mask.**
`presentation.cpp:1532`: `queue.entries[queue.read_cursor]`, where `entries` is
`std::array<std::uint8_t,32>` and `read_cursor` is a `std::uint8_t`. Every other consumer in
`movement.cpp` masks with `&31U`. Both state readers reject `read_cursor > 31`
(`movement.cpp:753`, `movement.cpp:1723`), so no validated state can reach it and both sanitizer
presets are green - but the function is public in `presentation.hpp` and the mask is free.

## 8. What I did not check

- **The recovery itself.** The table's base and stride from access records, the ASCII-to-tile
  arithmetic, the two tilemap rows, the BG3 / `$1800` measurement, the pack's additivity, the
  queue-probe event sets and the window-member order. The first two rounds reproduced all of it
  from the ROM and the audited core; this candidate changes none of that code or content. I did
  re-derive the table's ROM identity and the whole glyph domain from the ROM and the pack
  (section 4).
- **I captured no original of my own.** Every original I measured against is an accepted capture in
  `local/evidence` or the primary's own `pictures-a`, `trick-roll` and `trick-twist`. My
  independence is in the questions asked, not in a fresh capture.
- **The synthetic suite as a separate run.** The five preset `ctest` suites cover 23 tests each and
  were 23/23; I did not re-run `tools/project.py` synthetic on top of them, and the candidate adds
  no source that could move it.
- **Hosted CI on the final tip.** Not run by me; the closeout requirement is unchanged, and
  `codex/*`-style branches skip it, so the primary must dispatch it explicitly.
- **Whether the original displays a caption for the voice range 72-87.** Still unmeasured, as at
  all three earlier rounds. I established that native renders all sixteen entries without a throw;
  whether the original shows them is a different question and no capture answers it.
- **The PPU configuration behind the red add.** I confirmed nothing beyond what round three
  measured; CGADSUB, CGWSEL, TM/TS and a fixed colour are all unrecovered.
- **`gate_identity` against a build made in another worktree or preset**, and against a `--since`
  that is not an ancestor of `HEAD`. Both are operator error rather than an attack, and neither is
  guarded.

## 9. What acceptance rests on

Measured, in this checkout, at this candidate:

- the eleven differential gates, **run**, 11/11 identical in status, restore count and `rows_sha256`
  to the reports at `6e0fad6`;
- five preset suites 23/23; both v1 contracts passed; hidden runs `rc=0` with 0 fallback frames;
  fuzz 382,535 updates and 0 aborts;
- the 274-frame ZOOM ZOO sweep at worst caption-band mismatch 0, and all four caption picture
  scripts at 2304/2304 on every frame;
- the bitset change pixel-neutral over 47,140,878 bytes of rendered frames;
- five of six mutations of the caption rule killed by the tracked test;
- every byte of all 255 caption entries mapping to a glyph inside the font sheet, the table
  byte-identical to the ROM at `$17:C9F4 + 16`.

Derived rather than measured, and named as such: nothing in the acceptance now depends on
`gate_identity`, because the gates ran. If a later task uses the shortcut, what it buys is an
inference from byte-identical inputs to identical rows, and F1 to F4 are the conditions on that
inference.

Declared limits, unchanged:

- no capture displays a voice entry (ids 72-87), so whether the original shows that range at all is
  unmeasured; native draws one, and R-0042 carries the limit;
- the red add is evidenced on ZOOM ZOO only - four frames, 59 pixels, eight sprite colours, the
  clamp reached three times - and the eight DRAGSTER trick frames carry 999 flat caption pixels and
  no blend;
- the PPU configuration that selects between the caption's two colour indices, CGRAM 22 for the
  flat ink and CGRAM 27 for the contribution to a sprite, is unrecovered;
- `j`, `k`, `q`, `x` and `z` are read from the font sheet and from the arithmetic, not from any
  original picture; so are the three punctuation tiles.

## 10. What the primary needs to do

1. F1 to F5, all in the shortcut and its documentation. F4 is the one that will bite someone else:
   the published command does not run. F1 is the one that matters to the shortcut's own claim.
2. F6 and F7: the README's pack stem, and a record that names `bf16faf`, says three review rounds
   happened, says the eleven gates were cited rather than run at the tip, and whose attempts table
   renders. Attempt 19 should also say that the fourth round ran the eleven gates and got the same
   digests, because that is now the evidence the acceptance row rests on.
3. A1 to A7 as you see fit. A1 is a sentence; A7 is a `&31U`.
4. The caption work itself needs nothing further. C1 through C8 and E2 through E10 are addressed,
   the rule survives every test three rounds and this one could construct, and the numbers
   reproduce exactly.
