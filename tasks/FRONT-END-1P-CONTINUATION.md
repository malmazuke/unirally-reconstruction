...
| Nothing moves | Gates on `689fefb` (`gates-689fefb.out`; the same on `d6115e4`; the head after it changes only comments and records): three presets build, ctest 26 of 26; the synthetic suite; both v1 contracts; the hidden runs; the eleven differential gates (the same rows digests); the equivalence sweep, 351 runs with 0 differences; both recompares identical; the main menu's and the one-player screens' comparisons (the setup `compare.py` now knows the race screens); 0 functions over 80 lines; the address index current |
