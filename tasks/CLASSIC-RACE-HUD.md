...
- Provider quota window/baseline timestamp, used/remaining or unknown, reserve and session
  ...
  Measured as the task ran: 66% after the first candidate's gates, 70% after two review rounds,
  72% after the third, **74% after the fourth**, with the five-hour window at 91% and resetting
  within the hour. Four returned rounds cost roughly two to three points each, so the fifth round
  and integration were dispatched against the six points left under the reserve; a sixth round
  would not have fitted and the fallback was to integrate with the open point declared.
...
| 26 (review 4) | The finish sequence can be placed at fixed offsets from the finish | The fourth review confirmed the queue rule from the ROM - `$81:818D` is the only instruction that sets `$0D17`, `$053F` has four writers and no clearer and never changes inside a race, and native's predicate disagrees with the ROM's own condition nowhere over 125 captures - and then attacked the offsets | The offsets are the queue's output **only while nothing competes for it**. When the opponent's counter steps on the update right after the player finishes, `$0D17` is set again while `$0EFB` is already zero, so `finish` is written a second time and every field behind it waits. Measured on two DRAGSTER races: 918 pixels over pictures 3215-3217, the clock still standing where native had blanked it. Not a regression - the residual of the first round's blocking finding in a case no round had measured, and invisible to every gate because the DRAGSTER kept frames jump 3213 to 3400 | Follow the queue instead of counting from the finish |
| 27 (fix) | Following the queue itself removes the offsets | `ClassicRaceHudClock` now keeps the pending set - left field, clock blank, player's time, opponent's time - and services the first pending field each update in the dispatcher's order, exactly as `$81:F357` returns. The renderer reads what the queue wrote rather than a delay | The competing-finish case is **pixel-identical over the whole picture** on all 13 frames, where the review measured 918 differing pixels; all four ZOOM ZOO consecutive sets stay at 0 differing HUD-row pixels. Three mutants of the predicate are killed by the tests (the sprint finish must hold, the opponent's step must dirty the field, the blank must consume an update); one earlier "survivor" was a false result - dropping a term left a variable unused, `-Werror` failed the build, and the old binary passed. Also from the review: the count is seven or eight writes a tour race and two a sprint, the comment in `presentation.cpp` names `$81:EB91`/`$81:EB93` correctly (the header and R-0043 still said "falls through" until the fifth round caught it), and `dragster_probe.py` names both riders when they step together | Re-run the sweeps and gates, then a fifth round |
| 28 (gates) | Following the queue keeps everything else | `gates.sh` on `547fbfa` and all five kept-frame sweeps | All pass: 23/23 ctest on each of the five presets, synthetic `rc=0 status=passed`, both v1 contracts, 0 rider-pose fallback frames on both hidden runs, fuzz 40 seeds / 79 races / 0 aborts, the eleven differential gates citable. Every kept-frame sweep is byte-identical to the previous rounds' | Fifth review round |
| 29 (interrupted) | The fifth round would finish inside the five-hour window | Dispatched it at 74% weekly with the five-hour window already at 91% | It did not: the reviewer **failed partway through** with the account's five-hour session limit, after building and starting its own comparisons but before writing a report. No verdict exists from it, and under D-0006 there is no self-approval fallback, so the round was re-dispatched once the window reset - with a tighter scope, because the weekly reserve then left about five points | Re-dispatch, then integrate on approval |
| 30 (approved) | The queue model holds under an independent check | The re-dispatched round recaptured `regression-landing-held-roll-a` itself (2,066 frames asserted against the frozen digests) and checked four more windows, the five kept-frame sweeps and three mutants | **Approve.** Pictures 3206-3224 are 0 differing pixels over the whole frame on all 19 - the reviewer checked six more frames than the primary claimed - and the glyphs show the mechanism rather than a coincidence: the original holds `finish ... 0:33:5` through 3215 and blanks at 3216, which is the second `finish` write, and native now does the same. No regression in any window; all three mutants killed from builds verified to compile first. Two should-fix items, both records: R-0043's sprint count contradicted the mechanism four lines above it, and row 27 claimed a ROM-citation fix that had reached `presentation.cpp` only | Apply both, then integrate |
