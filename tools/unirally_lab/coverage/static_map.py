...
def listing(a: Analysis, cls: list[str], bank: int, names_by_offset: dict[int, str], xrefs: dict[int, list[str]]) -> str:
...
def build(a: Analysis, cls: list[str], root: Path, map_paths: list[Path], coverage_digests: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    doc = {
        ...
        "totals": {"bytes": SPAN, **totals, "unknown_share": round(totals["unknown"] / SPAN, 4),
                   ...
                   "cited_addresses": len(labs), "cited_by_class": dict(sorted(Counter(l["class"] for l in labs).items())),
...
def summary_markdown(doc: dict[str, Any], labs: list[dict[str, Any]]) -> str:
    ...
    for k in ("routines", "gap_sweep_candidate_instructions", "gap_sweep_candidate_bytes", "jump_tables", "mode_ambiguities", "conflicts", "data_references_into_code",
