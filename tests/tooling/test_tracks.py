...
class PackProfileTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        self.assertEqual(rules["profile_id"], "classic.pal.crawler.tracks.v21")
...
class FrontEndEntryTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        later += len(front_end.RESULT_TABLES) + len(front_end.LAP_RESULT_TABLES) + len(front_end.AWARD_ASSETS) + len(front_end.AWARD_TABLES) + len(front_end.RACE_PAIRING_TABLES)
...
class RiderMenuEntryTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        tour_menu += len(front_end.RESULT_TABLES) + len(front_end.LAP_RESULT_TABLES) + len(front_end.AWARD_ASSETS) + len(front_end.AWARD_TABLES) + len(front_end.RACE_PAIRING_TABLES)
...
class ResultEntryTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        later = len(front_end.LAP_RESULT_TABLES) + len(front_end.AWARD_ASSETS) + len(front_end.AWARD_TABLES) + len(front_end.RACE_PAIRING_TABLES)
        added = rules["entries"][-len(expected) - later:-later]
        self.assertEqual([e["id"] for e in added], expected)
        source = (ROOT / "src" / "core" / "content_pack.cpp").read_text(encoding="utf-8")
...
class LapResultEntryTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        later = len(front_end.AWARD_ASSETS) + len(front_end.AWARD_TABLES) + len(front_end.RACE_PAIRING_TABLES)
        added = rules["entries"][-len(expected) - later:-later]
        self.assertEqual([e["id"] for e in added], expected)
        source = (ROOT / "src" / "core" / "content_pack.cpp").read_text(encoding="utf-8")
...
class AwardEntryTests(unittest.TestCase):
    ...
    def test_rules_and_compiled_table_agree(self) -> None:
        ...
        later = len(front_end.RACE_PAIRING_TABLES)
        added = rules["entries"][-len(expected) - later:-later]
        self.assertEqual([e["id"] for e in added], expected)
        source = (ROOT / "src" / "core" / "content_pack.cpp").read_text(encoding="utf-8")
