...
int main() {
    ...
    // Its state carries its own identity, URTR13 04 (the 742-byte layout, the
    // special-tile words of R-0047 with $0C73, and the checkpoint flags of
    // R-0048), and round-trips.
    ...
    const std::array<std::uint8_t,8> flat_magic{'U','R','T','R','1','3','0','4'};
    require(flat_bytes.size()==838 && classic_race_state_magic(flat_fun)==flat_magic && std::equal(flat_magic.begin(),flat_magic.end(),flat_bytes.begin()));
