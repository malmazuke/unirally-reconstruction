...
std::size_t banked_offset(std::uint8_t bank, std::uint16_t address,
                          std::uint8_t first_bank) {
  if (bank < first_bank || address < 0x8000U)
    throw std::invalid_argument("rider object reference precedes its packed banks");
  return static_cast<std::size_t>(bank - first_bank) * bank_bytes +
         (address - 0x8000U);
}
...
FrameStream open_frame(const RiderObjectContent &content, std::uint16_t pose,
                       RiderRowClip clip) {
  ...
  frame.row_masks = {
      ...
      static_cast<std::uint8_t>(((header[1] & 0x0fU) << 4U) | ((static_cast<unsigned>(header[2]) >> 6U) << 2U)),
  ...
  const std::size_t words =
      static_cast<std::size_t>(std::popcount(frame.row_masks[0]) + std::popcount(frame.row_masks[1]) +
...
void copy_tile(const RiderObjectContent &content, std::uint16_t word,
               RiderObjectPixels &pixels, std::size_t row, std::size_t column) {
...
RiderObjectPixels compose_rider_object(const RiderObjectContent &content,
  ...
  FrameStream overlay;
  if (overlay_pose)
    overlay = open_frame(content, *overlay_pose, clip);
...
RiderOam project_rider_oam(std::uint16_t world_x, std::uint16_t world_y,
  ...
  if (bit_below(scaled, left_limit) || !bit_below(scaled, right_limit))
    return oam;
  oam.visible = true;
