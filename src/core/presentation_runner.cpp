...
std::vector<std::uint8_t> read(const std::filesystem::path& p) {
    ...
    if (!in) throw std::runtime_error("cannot open presentation state");
...
int main(int argc, char** argv) try {
    ...
    const auto frame =
        unirally::render_dragster_headless({state, camera, sx, sy, bg2x, bg2y}, assets);
    std::ofstream out(out_path, std::ios::binary | std::ios::trunc);
    if (!out) throw std::runtime_error("cannot create presentation output");
    out << "P6\n256 224\n255\n";
    out.write(reinterpret_cast<const char*>(frame.pixels.data()),
              static_cast<std::streamsize>(frame.pixels.size()));
    if (!out) throw std::runtime_error("cannot write presentation output");
    return 0;
