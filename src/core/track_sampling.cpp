...
CollisionPoints collision_points(const SamplingContent& content, std::uint16_t pose_index,
                                 bool reflected) {
    ...
    for (std::size_t i = 0; i < 2; ++i) {
        points[i] = {byte(content.poses, pose + i * 2), byte(content.poses, pose + i * 2 + 1)};
    }
    ...
    const unsigned template_offset =
        ((((static_cast<unsigned>(selector) & 0xFFU) << 8) | (static_cast<unsigned>(selector) >> 8))
         << 4)
        & 0xFFFFU;
    for (std::size_t i = 0; i < 8; ++i) {
        points[i + 2] = {
            static_cast<std::uint8_t>(origin_x + byte(content.templates, template_offset + i * 2)),
            static_cast<std::uint8_t>(origin_y
                                      + byte(content.templates, template_offset + i * 2 + 1))};
    }
...
TrackSamples sample_track(const SamplingContent& content, const CollisionPoints& points,
    ...
    const std::array<unsigned, 4> addresses = {upper_left, right, (upper_left + stride) & 0xFFFFU,
                                               (right + stride) & 0xFFFFU};
    for (std::size_t i = 0; i < blocks.size(); ++i) {
        // The header is 15 bytes ($7F:000F); each fine-cell block is 32 bytes.
        blocks[i] = (static_cast<unsigned>(word(content.track, coarse_map_offset + addresses[i]))
                     * block_bytes)
                  & 0xFFFFU;
    }
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
        ...
        const unsigned fine_x =
            ((static_cast<unsigned>(point.x) + position_x) & fine_cell_mask) >> 2;
