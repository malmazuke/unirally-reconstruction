...
std::uint16_t wrap(unsigned value) { return static_cast<std::uint16_t>(value); }
...
HeadPoint head_point(const ZoomZooState &state, std::size_t rider,
  ...
  const auto points = collision_points(content.movement.sampling,
                                       movement.pose.pose_index, movement.pose.reflected);
...
std::optional<std::uint16_t> height_target(const RiderLookTables &tables,
                                           std::uint16_t distance, HeadPoint own,
                                           HeadPoint other) {
  ...
  if (compares_negative(own.y, other.y)) {
    for (std::uint16_t target = 9;; --target) {
      ...
      if (!compares_negative(height, other.y))
        return target;
      if (compares_negative(wrap(target - 1U), 1))
        return std::nullopt;
    }
  }
  for (std::uint16_t target = 9;; ++target) {
    accumulated = wrap(accumulated + table_word(tables, height_step_table + distance));
    height = wrap(height - accumulated);
    if (compares_negative(height, other.y) || height == other.y)
      return target;
    if (target + 1U == 0x12)
      return std::nullopt;
  }
...
void look_for_rider(RiderLook &look, RiderLook &player_look, std::size_t rider,
  ...
  if (look.sequence_cursor == 0 && rider == 1) {
    ...
    if (look.sequence_number == 6)
      look.sequence_number = 0;
  ...
  if (look.sequence_delay == 0) {
    if (look.sequence_cursor == look.sequence_end) {
      ...
      look.sequence_target = static_cast<std::uint16_t>((look.sequence_target & 0xff00U) | (entry & 0xffU));
      look.target = static_cast<std::uint16_t>((look.target & 0xff00U) | (entry & 0xffU));
      look.sequence_delay = static_cast<std::uint16_t>((look.sequence_delay & 0xff00U) | (entry >> 8U));
...
rider_overlay_poses(const RiderLookState &look, const ZoomZooState &updated,
  ...
  for (std::size_t rider = 0; rider < 2; ++rider) {
    ...
    if (updated.reflection[rider].pose_override != 0 || head == 0)
      continue;
    ...
    if (!compares_negative(orientation, 0x31)) {
      ...
      const auto side_base =
          table_word(tables, side_overlay_base_table + 2U * (static_cast<std::size_t>(head) - 1U));
