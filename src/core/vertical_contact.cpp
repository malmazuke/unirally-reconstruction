...
void resolve_vertical_contact(RiderContactState& rider, ContactMotion& motion,
    ...
    require(rider.unsupported_count <= 9 && probed.penetration < 128,
    ...
    if (next.auxiliary_flag == 1) {
        ...
        next.unsupported_count =
            std::min<std::uint16_t>(9, static_cast<std::uint16_t>(incoming.unsupported_count + 1U));
        rider = next;
        return;
    }
    if (!summary.supported) {
        ...
        require(magnitude < shifts.size(),
                "vertical response angle outside recovered coefficients");
        // $81:924E–9275 removes motion into the inverted contact face.
        if (signed_word(moved.velocity_y) < 0 && (summary.selected_high & 0x80U)
            && ((summary.selected_high & 0x40U) ? signed_word(moved.velocity_x) < 0
                                                : signed_word(moved.velocity_x) >= 0))
            moved.velocity_x = 0;
        next.surface_angle = static_cast<std::uint16_t>(summary.angle);
        next.angle_unspecified = magnitude == 31;
        next.unsupported_count = 0;
        next.unsupported_duration = 0;
        if (magnitude >= 31) {
            moved.velocity_x = arithmetic_shift(moved.velocity_x, 2);
            next.unsupported_count = std::min<std::uint16_t>(
                9, static_cast<std::uint16_t>(incoming.unsupported_count + 1U));
            next.unsupported_duration =
                static_cast<std::uint16_t>(incoming.unsupported_duration + 1U);
        } else if (incoming.unsupported_count >= 9
                   || (summary.leading_support && incoming.unsupported_count >= 2)) {
            ...
            if (magnitude < 28 && flag_pair != 8) {
                ...
                const auto dx = std::abs(signed_word(
                    static_cast<std::uint16_t>(motion.x - incoming.previous_uncorrected_x)));
                const auto half_dy = std::abs(signed_word(static_cast<std::uint16_t>(
                                         motion.y - incoming.previous_uncorrected_y)))
                                   / 2;
                // $81:984D-98A8 uses bounded subtraction, not division. A zero
                // subtrahend still terminates at the angle endpoint (4 or 31).
                const int magnitude_angle =
                    dx >= half_dy ? (half_dy == 0 ? 4 : std::max(4, 16 - 4 * (dx / half_dy)))
                                  : (dx == 0 ? 31 : std::min(31, 16 + 4 * (half_dy / dx)));
                const int coarse = signed_word(static_cast<std::uint16_t>(
                                       motion.x - incoming.previous_uncorrected_x))
                                         < 0
                                     ? -magnitude_angle
                                     : magnitude_angle;
                ...
                const int surface_direction =
                    summary.angle < 0 ? 62 + summary.angle : summary.angle;
                const bool negative_dx = signed_word(static_cast<std::uint16_t>(
                                             motion.x - incoming.previous_uncorrected_x))
                                       < 0;
                const bool negative_dy = signed_word(static_cast<std::uint16_t>(
                                             motion.y - incoming.previous_uncorrected_y))
                                       < 0;
                int orientation_angle = (negative_dx != negative_dy) ? -coarse : coarse;
                orientation_angle =
                    (!negative_dx && !negative_dy)
                ...
                const int coarse_direction =
                    orientation_angle < 0 ? 62 + orientation_angle : orientation_angle;
                ...
                if (direction_difference != 0 && direction_difference != 31) {
                    ...
                    if (direction_difference > 31) {
                        response = (displacement < 3 && pose_direction < 32)
                                     ...
                                     : (displacement < 1
                                            ...
                                            : std::max(-2, -1
                                                               - static_cast<int>(
                                                                   motion.previous_x_displacement
                                                                   >> 4U)));
                    } else {
                        response = (displacement < 3 && pose_direction > 32)
                                     ? 1
                                     : (displacement < 1
                                            ...
                                            : std::min(2, static_cast<int>(
                                                              motion.previous_x_displacement >> 4U)
                                                              + 1));
                    }
                    ...
                    if (!summary.leading_support) {
                        if (response) {
                            const auto impulse = static_cast<std::uint16_t>(
                                (motion.previous_x_displacement >> 2U) + 1U);
                            moved.orientation_impulse =
                                response < 0 ? static_cast<std::uint16_t>(0U - impulse) : impulse;
                        }
                        if (context.mode == 0 && summary.angle < 30 && summary.angle >= -30
                            && incoming.unsupported_duration >= 120 && pose_direction >= 32) {
                    ...
                    auto decay = [](std::uint16_t value) {
                        return static_cast<std::uint16_t>(
                            value + (signed_word(value) < 0 ? 1 : (value ? -1 : 0)));
                    };
                    moved.response_a = decay(moved.response_a);
                    moved.response_b = decay(moved.response_b);
                }
                ...
                if (angle_difference > 5 || summary.leading_support || force_long_airtime_matrix
                    || context.landing_matrix_zero) {
                    require(landing_matrices.size() == 1512,
                            "landing coefficient matrices are missing");
                    unsigned bucket =
                        angle_difference
                            ? static_cast<unsigned>(std::min(3, (angle_difference - 1) / 5))
                            : 0U;
                    ...
                    moved.velocity_y =
                        static_cast<std::uint16_t>(multiply(vy, 0) + multiply(vx, 1));
                    moved.velocity_x =
                        static_cast<std::uint16_t>(multiply(vy, 2) + multiply(vx, 3));
                }
            } else
                next.recontact = true;
        } else {
            if (!summary.leading_support && context.phase == 0) {
                moved.response_a = 0;
                moved.response_b = 0;
            }
            const auto shifted = arithmetic_shift(motion.velocity_x, byte(shifts, magnitude));
            const auto product = static_cast<std::uint16_t>(static_cast<unsigned>(shifted)
                                                            * byte(multipliers, magnitude));
            // $81:96FA-970B: under surface mode pair 8 keeps velocity y.
            if (!summary.leading_support && magnitude < 26 && !(context.mode && flag_pair == 8)) {
                moved.velocity_y =
                    summary.angle < 0 ? static_cast<std::uint16_t>(1U - product) : product;
                if (summary.selected_high & 0x80U)
                    moved.velocity_y = static_cast<std::uint16_t>(0U - moved.velocity_y);
            }
            const int contribution = summary.angle < 0 ? -static_cast<int>(magnitude / 2U)
                                                       : static_cast<int>(magnitude / 2U);
            if (!summary.leading_support && magnitude < 28 && !(summary.selected_high & 0x80U))
                moved.velocity_x =
                    static_cast<std::uint16_t>(static_cast<int>(motion.velocity_x) + contribution);
        }
        ...
        if (magnitude == 28 && !summary.leading_support && incoming.unsupported_count < 9) {
            ...
            auto velocity = static_cast<std::uint16_t>(static_cast<unsigned>(shifted)
                                                       * byte(multipliers, magnitude));
            ...
            moved.velocity_x = (summary.selected_high & 0x80U)
                                 ? static_cast<std::uint16_t>(0U - velocity)
                                 : velocity;
        }
    }
