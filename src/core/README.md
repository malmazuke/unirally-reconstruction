...
state is `URTRnn04`: that layout followed by the 36 special-tile bytes (R-0047, with the
turnaround `$0C73` of R-0050) and the
last 60 checkpoint-seen flags of R-0048 (838 bytes). The special-tile bytes
DRAGSTER and ZOOM ZOO also append, as `URDG0003` and `URZZ000D`, only while one of those
