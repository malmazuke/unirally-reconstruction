...
constexpr std::uint16_t unrecovered_ai_level = 2;
...
bool player_lets_launch(const ZoomZooState& state) {
    const auto& whole = state.movement;
    const auto lead = static_cast<std::int16_t>(whole.riders[0].progress.transition_count
                                                - whole.riders[1].progress.transition_count);
    return whole.rewards.feature_total == 0 || lead >= launch_lead;
}
...
void launch_trick(ZoomZooState& state, unsigned ai_level) {
    auto& rider = state.movement.riders[1];
    auto& ai = state.movement.opponent_ai;
    ai.suppression_counter =
        ai_level > unrecovered_ai_level ? hunter_rotation_updates : rotation_updates;
...
MarkerJump jump_at_marker(ZoomZooState& state) {
    ...
    if (rider.contact.unsupported_count >= launch_airborne_updates
        ...
        // Below level 2 ($83:E1A8) the player must let it launch; above it (the HUNTER
        // tour, level 3) it always launches (LOCKED-TOURS). Level 2 is on no observed
        // track.
        if (level == unrecovered_ai_level) throw std::invalid_argument("AI level 2 is unrecovered");
        if (level > unrecovered_ai_level || player_lets_launch(state)) {
            launch_trick(state, level);
            return MarkerJump::launched;
        }
        ai.suppression_counter = 0;
        return MarkerJump::drop_jump;
    }
    // $83:E135-E13D: an AI level other than 1 (HUNTER's 3) takes the $83:E222 path at
    // once, keeping the jump (LOCKED-TOURS).
    if (rider.contact.unsupported_count < launch_airborne_updates
