...
inline ZoomZooContent zoom_zoo_content(const ClassicContentPack& pack) {
    ...
    const auto engine = [&](const char* name) {
        return pack.entry(std::string("physics.") + name);
    };
    return {
        {{zoom("track-data"), engine("rider.collision-poses"), engine("rider.collision-templates")},
         {zoom("tile-tables"), zoom("tile-flags")},
         engine("track.progress-transitions"),
         engine("rider.pose-slopes"),
         engine("rider.displacement-table"),
         engine("rider.idle-pose-table"),
         zoom("race-finish-reward-values"),
         zoom("race-finish-reward-classes"),
         {engine("speed.masks"), engine("speed.decrements")}},
        zoom("sustained-slope-coefficients"),
        zoom("reflection-pose-table"),
        zoom("landing-response-matrices"),
        zoom("race-finish-poses"),
        zoom("roll-pose-table"),
        zoom("roll-direction-table"),
        zoom("roll-reward-weights"),
        zoom("trick-combinations"),
...
inline std::string classic_track_entry(ClassicRaceTrack track, const char* part) {
    return std::string("track.") + char('0' + track.index / 10U) + char('0' + track.index % 10U)
         + '.' + part;
}
